...
const hGroup GroupBase::HGROUP_REFERENCES = {1};
...
//             Vector ut = SK.GetEntity(g.predef.entityB)->VectorGetNum(SK);
//             Vector vt = SK.GetEntity(g.predef.entityC)->VectorGetNum(SK);
...
//             }
//             if(SS.GW.projRight.Dot(ut) < 0)
//                 g.predef.negateU = true;
//             if(SS.GW.projUp.Dot(vt) < 0)
...
//                 g.predef.q      = normal->NormalGetNum(SK);
//             } else {
...
//                 } else if(wrkplg->subtype ==
//                 Subtype::WORKPLANE_BY_POINT_ORTHO) {
...
//             g.predef.q       = SK.GetEntity(gs.anyNormal[0])->NormalGetNum(SK);
//             g.predef.origin  = gs.point[0];
//             //} else if(gs.faces == 1 && gs.points == 1 && gs.n == 2) {
//             //    g.subtype = Subtype::WORKPLANE_BY_POINT_FACE;
//             //    g.predef.q      = SK.GetEntity(gs.face[0])->NormalGetNum(SK);
//             //    g.predef.origin = gs.point[0];
//         } else {
//             Error(_("Bad selection for new sketch in workplane. This "
//                     "group can be created with:\n\n"
//                     "    * a point (through the point, orthogonal to
//                     coordinate axes)\n" "    * a point and two line segments
//                     (through the point, " "parallel to the lines)\n" "    * a
//                     point and a normal (through the point, " "orthogonal to
//                     the normal)\n"
...
//         if(!SS.GW.LockedInWorkplane()) {
//             Error(_("Lathe operation can only be applied to planar
//             sketches.")); return;
//         }
...
//         if(!SS.GW.LockedInWorkplane()) {
//             Error(_("Revolve operation can only be applied to planar
//             sketches.")); return;
//         }
//         if(gs.points == 1 && gs.vectors == 1 && gs.n == 2) {
//             g.predef.origin  = gs.point[0];
...
//         if(!SS.GW.LockedInWorkplane()) {
//             Error(_("Helix operation can only be applied to planar
//             sketches.")); return;
//         }
//         if(gs.points == 1 && gs.vectors == 1 && gs.n == 2) {
//             g.predef.origin  = gs.point[0];
...
//             g.predef.entityB  = w->Normal(SK)->h;
...
//             Platform::CreateOpenFileDialog(SS.GW.window);
...
//     return forceToMesh || IsTriangleMeshAssembly() ||
//     IsForcedToMeshBySource();
// }
...
// void Group::Generate(IdList<Entity, hEntity> *entity, IdList<Param, hParam>
// *param) {
...
//             Vector u = SK.GetEntity(predef.entityB)->VectorGetNum(SK);
//             Vector v = SK.GetEntity(predef.entityC)->VectorGetNum(SK);
//             u        = u.WithMagnitude(1);
//             Vector n = u.Cross(v);
...
//             q = SK.GetEntity(predef.entityB)->NormalGetNum(SK);
//         } else
//             ssassert(false, "Unexpected workplane subtype");
...
//         point.numPoint     = SK.GetEntity(predef.origin)->PointGetNum(SK);
...
//         hEntity pt = {0};
//         // Not using range-for here because we're changing the size of entity
//         in the loop. for(i = 0; i < entity->n; i++) {
...
//             h.param(0), h.param(1),
//                        h.param(2), NO_PARAM, NO_PARAM, NO_PARAM, NO_PARAM,
//                        NO_PARAM, CopyAs::N_TRANS);
//             CopyEntity(entity, SK.GetEntity(he), af, REMAP_TOP, h.param(0),
//             h.param(1), h.param(2),
...
//     case Type::LATHE: {
//         Vector axis_pos = SK.GetEntity(predef.origin)->PointGetNum(SK);
//         Vector axis_dir = SK.GetEntity(predef.entityB)->VectorGetNum(SK);
...
//                 CopyEntity(entity, SK.GetEntity(he), 0,
//                 REMAP_LATHE_ARC_CENTER, NO_PARAM, NO_PARAM,
//                            NO_PARAM, NO_PARAM, NO_PARAM, NO_PARAM, NO_PARAM,
//                            NO_PARAM, CopyAs::NUMERIC);
...
//             // Arcs are not generated for revolve groups, for now, because
//             our current arc
//             // entity is not chiral, and dragging a revolve may break the arc
//             by inverting it.
//             // MakeLatheCircles(entity, param, he, axis_pos, axis_dir);
//             MakeLatheSurfacesSelectable(entity, he, axis_dir);
//         }
...
//             h.param(1), h.param(2),
//                        h.param(3), h.param(4), h.param(5), h.param(6),
//                        h.param(7), CopyAs::N_ROT_AXIS_TRANS);
...
//             if(e->IsPoint()) {
//                 Vector check =
//                 e->PointGetNum(SK).Minus(axis_pos).Cross(axis_dir);
...
//         for(a = a0; a < n; a++) {
//             // Not using range-for here because we're changing the size of
//             entity in the loop. for(i = 0; i < entity->n; i++) {
//                 Entity *e = &(entity->Get(i));
//                 if(e->group != opA)
//                     continue;

//                 e->CalculateNumerical(/*forExport=*/false);
//                 CopyEntity(entity, e, a * 2 - (subtype == Subtype::ONE_SIDED
//                 ? 0 : (n - 1)),
//                            (a == (n - 1)) ? REMAP_LAST : a, h.param(0),
//                            h.param(1), h.param(2), NO_PARAM, NO_PARAM,
//                            NO_PARAM, NO_PARAM, NO_PARAM, CopyAs::N_TRANS);
//             }
//         }
//         return;
//     }
...
//                 ? 0 : (n - 1)),
//                            (a == (n - 1)) ? REMAP_LAST : a, h.param(0),
//                            h.param(1), h.param(2), h.param(3), h.param(4),
//                            h.param(5), h.param(6), NO_PARAM,
//                            CopyAs::N_ROT_AA);
//             }
//         }
//         return;
...
//         // Not using range-for here because we're changing the size of entity
//         in the loop. for(i = 0; i < impEntity.n; i++) {
//             Entity *ie = &(impEntity[i]);
//             CopyEntity(entity, ie, 0, 0, h.param(0), h.param(1), h.param(2),
//             h.param(3), h.param(4),
//                        h.param(5), h.param(6), NO_PARAM,
//                        CopyAs::N_ROT_TRANS);
//         }
...
//            (this->allowRedundant && this->solved.how ==
//            SolveResult::REDUNDANT_OKAY);
// }
...
void GroupBase::GenerateEquations(IdList<Equation, hEquation> *l) {}
...
//     } else if(type == Type::ROTATE || type == Type::REVOLVE || type ==
//     Type::HELIX) {
...
//         Vector axis = SK.GetEntity(predef.entityB)->VectorGetNum(SK);
...
//         if(predef.entityB != EntityBase::FREE_IN_3D) {
//             // The extrusion path is locked along a line, normal to the
//             // specified workplane.
//             EntityBase *w           = SK.GetEntity(predef.entityB);
//             ExprVector u        = w->Normal(SK)->NormalExprsU();
//             ExprVector v        = w->Normal(SK)->NormalExprsV();
//             ExprVector extruden = {Expr::From(h.param(0)),
//             Expr::From(h.param(1)),
//                                    Expr::From(h.param(2))};
...
//         if(predef.entityB != EntityBase::FREE_IN_3D) {
//             EntityBase *w    = SK.GetEntity(predef.entityB);
//             ExprVector n = w->Normal(SK)->NormalExprsN();
//             ExprVector trans;
...
//     if(it == remap.end()) {
//         std::tie(it, std::ignore) = remap.insert({{in, copyNumber},
//         {(uint32_t)remap.size() + 1}});
//     }
...
//         // original line is a point in the plane, and the line is in the
//         plane. Vector a  = SK.GetEntity(ep->point[0])->PointGetNum(SK); Vector
//         b  = SK.GetEntity(ep->point[1])->PointGetNum(SK); Vector ab =
//         b.Minus(a);
...
// void Group::MakeLatheCircles(IdList<Entity, hEntity> *el, IdList<Param,
// hParam> *param, hEntity in,
...
//         // Project arc point to the revolution axis and use it for arc
//         center. double k     = pp->numPoint.Minus(pt).Dot(axis) /
//         axis.Dot(axis); pc->numPoint = pt.Plus(axis.ScaledBy(k));
...
// void Group::MakeLatheSurfacesSelectable(IdList<Entity, hEntity> *el, hEntity
// in, Vector axis) {
//     Entity *ep = SK.GetEntity(in);
...
//         Vector a = SK.GetEntity(ep->point[0])->PointGetNum(SK);
//         Vector b = SK.GetEntity(ep->point[1])->PointGetNum(SK);
...
// void Group::MakeRevolveEndFaces(IdList<Entity, hEntity> *el, hEntity pt, int
// ai, int af) {
//     if(pt.v == 0)
//         return;
...
//     // When there is no loop normal (e.g. if the loop is broken), use normal
//     of workplane
//     // as fallback, to avoid breaking constraints depending on the faces.
//     if(n.Equals(Vector::From(0.0, 0.0, 0.0)) && src->type ==
//     Group::Type::DRAWING_WORKPLANE) {
//         n = SK.GetEntity(src->h.entity(0))->Normal(SK)->NormalN(SK);
//     }
...
// void Group::MakeExtrusionTopBottomFaces(IdList<Entity, hEntity> *el, hEntity
// pt) {
//     if(pt.v == 0)
//         return;
//     Group *src = SK.GetGroup(opA);
...
// void Group::CopyEntity(IdList<Entity, hEntity> *el, Entity *ep, int
// timesApplied, int remap,
//                        hParam dx, hParam dy, hParam dz, hParam qw, hParam
//                        qvx, hParam qvy, hParam qvz, hParam dist, CopyAs as) {
//     Entity en       = {};
//     en.type         = ep->type;
...
//         } else { // N_ROT_AXIS_TRANS probably doesn't warrant a new entity
//         Type
...
//         EntReqTable::GetEntityInfo(ep->type, ep->extraPoints, NULL, &points,
//         &hasNormal,
//                                    &hasDistance);
//         for(i = 0; i < points; i++) {
...
//     return SS.explode && h == SS.GW.activeGroup && type ==
//     Type::DRAWING_WORKPLANE &&
//            !SS.exportMode;
// }
